...
import { ChangeFileUpload } from './ChangeFileUpload'
...
export const ChangeFileList = ({ projectId }: ChangeFileListProps) => {
  ...
  const {
    ...
    queryFn: async () => {
      const data = await productionService.getProductionFilesByStage(projectId, 'change')
      return data
    },
    enabled: !!projectId,
  })
    ...
    if (files.length === 0) {
      return (
        <div style={{ textAlign: 'center', padding: '60px 20px', color: '#999' }}>
          <Empty description="暂无文件，请上传变更洽商文件" image={Empty.PRESENTED_IMAGE_SIMPLE} />
        </div>
      )
    }
        ...
        {isEmpty ? (
          <div style={{ textAlign: 'center', padding: '60px 20px', color: '#999' }}>
            ...
            <div style={{ color: '#666', marginBottom: '20px' }}>
              {canManage ? '请点击右上角"+ 上传文件"按钮上传变更洽商文件' : '暂无变更洽商文件'}
            </div>
          </div>
        ) : (
