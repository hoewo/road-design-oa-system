...
interface PreliminaryStageEditModalProps {
...
export const PreliminaryStageEditModal = ({
  projectId,
  visible,
  stageInfo,
  onCancel,
  onSuccess,
}: PreliminaryStageEditModalProps) => {
  ...
  const { data: latestStageInfo, isLoading: isLoadingLatest, refetch: refetchLatest } = useQuery<StageFileInfo>({
    ...
    queryFn: async () => {
      console.log('[PreliminaryStageEditModal] 开始获取最新数据, projectId:', projectId)
      const data = await productionService.getProductionFilesByStage(projectId, 'preliminary')
      console.log('[PreliminaryStageEditModal] 获取到最新数据:', data)
      return data
    },
  ...
  const uploadMainFileMutation = useMutation({
    mutationFn: async (params: { file: globalThis.File; reviewSheetFile?: globalThis.File; reviewSheetFileId?: string; score?: number }) => {
      console.log('[PreliminaryStageEditModal] 开始上传主文件:', {
      ...
      formData.append('file_type', 'preliminary_design')
      formData.append('stage', 'preliminary')
      
      // 如果提供了校审单文件 ID（引用已存在的校审单），优先使用
      if (params.reviewSheetFileId) {
        formData.append('review_sheet_file_id', params.reviewSheetFileId)
        console.log('[PreliminaryStageEditModal] 使用已存在的校审单文件 ID:', params.reviewSheetFileId)
      } else if (params.reviewSheetFile) {
        // 如果提供了新的校审单文件，上传新文件
        formData.append('review_sheet_file', params.reviewSheetFile)
        console.log('[PreliminaryStageEditModal] 上传新的校审单文件:', params.reviewSheetFile.name)
      }
      
      // 如果提供了评分，同时传递
      if (params.score !== undefined && params.score !== null) {
        formData.append('score', String(params.score))
        console.log('[PreliminaryStageEditModal] 同时上传评分:', params.score)
      }
      
      const result = await productionService.uploadProductionFile(projectId, formData)
      console.log('[PreliminaryStageEditModal] 主文件上传成功:', result)
      return result
    },
    onError: (error: any) => {
      console.error('[PreliminaryStageEditModal] 主文件上传失败:', error)
  ...
  const uploadReviewSheetMutation = useMutation({
    mutationFn: async (params: { file: globalThis.File; score?: number; hasPendingMainFiles?: boolean }) => {
      console.log('[PreliminaryStageEditModal] 开始上传校审单:', {
        fileName: params.file.name,
        score: params.score,
      ...
      if (existingReviewSheet) {
        ...
        console.log('[PreliminaryStageEditModal] 校审单记录更新成功')
      ...
      const result = await productionService.uploadProductionFile(projectId, formData)
      console.log('[PreliminaryStageEditModal] 校审单记录创建成功:', result)
      return result
    },
    onError: (error: any) => {
      console.error('[PreliminaryStageEditModal] 校审单上传失败:', error)
  ...
  const updateScoreMutation = useMutation({
    mutationFn: async (score: number) => {
      console.log('[PreliminaryStageEditModal] 开始更新评分:', score)
      const result = await productionService.updateStageScore(projectId, 'preliminary', score)
      console.log('[PreliminaryStageEditModal] 评分更新成功')
      return result
    },
    onError: (error: any) => {
      console.error('[PreliminaryStageEditModal] 评分更新失败:', error)
  ...
  const handleSubmit = async () => {
    try {
      console.log('[PreliminaryStageEditModal] ========== 开始保存 ==========')
      const values = await form.validateFields()
      console.log('[PreliminaryStageEditModal] 表单验证通过，表单值:', values)
      ...
      // 初步设计阶段要求：如果该阶段还没有校审单记录，则必须提供校审单和评分
      ...
      if (newMainFiles.length > 0) {
        ...
        if (!reviewSheetFile && !hasExistingReviewSheet) {
          message.error('初步设计阶段必须提供校审单，请先上传校审单')
          setUploading(false)
          return
        }
        // 如果创建新的校审单记录，必须提供评分
      ...
      if (reviewSheetFile) {
        ...
        console.log('[PreliminaryStageEditModal] 先上传校审单（因为需要先创建校审单记录）')
        await uploadReviewSheetMutation.mutateAsync({
          file: reviewSheetFile,
          score,
          hasPendingMainFiles: newMainFiles.length > 0, // 传递是否有待上传的主文件
        })
        console.log('[PreliminaryStageEditModal] 校审单上传完成，现在可以上传主文件')
      }

      // 上传主文件（不包含校审单，因为主文件和校审单是分开的记录）
      for (const fileItem of newMainFiles) {
        if (fileItem.originFileObj) {
          console.log('[PreliminaryStageEditModal] 上传主文件，已有校审单:', hasExistingReviewSheet || !!reviewSheetFile)
      ...
      // 刷新查询缓存，确保数据更新
      console.log('[PreliminaryStageEditModal] 开始刷新查询缓存')
      await queryClient.invalidateQueries({
        queryKey: ['productionFilesByStage', projectId, 'preliminary'],
      })
      console.log('[PreliminaryStageEditModal] 查询缓存已刷新')

      // 重新获取最新数据
      console.log('[PreliminaryStageEditModal] 开始重新获取最新数据')
      const refreshedData = await refetchLatest()
      console.log('[PreliminaryStageEditModal] 重新获取到的数据:', refreshedData.data)
      ...
      console.log('[PreliminaryStageEditModal] 调用 onSuccess 回调')
      onSuccess()
    } catch (error) {
      setUploading(false)
      console.error('[PreliminaryStageEditModal] 保存失败:', error)
