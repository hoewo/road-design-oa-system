...
import { SchemeStageEditModal } from './SchemeStageEditModal'
import type { StageFileInfo, ProductionFile } from '@/types'
import dayjs from 'dayjs'

const { Text } = Typography

interface SchemeStageFileManagementProps {
  projectId: string | number
}

export const SchemeStageFileManagement = ({
  projectId,
}: SchemeStageFileManagementProps) => {
  ...
  const {
    ...
    queryFn: async () => {
      console.log('[SchemeStageFileManagement] 开始获取阶段文件信息, projectId:', projectId)
      const data = await productionService.getProductionFilesByStage(projectId, 'scheme')
      console.log('[SchemeStageFileManagement] 获取到的数据:', data)
      return data
    },
    enabled: !!projectId,
  })
  ...
  // 日志：数据状态变化
  useEffect(() => {
    console.log('[SchemeStageFileManagement] 数据状态:', {
      projectId,
      isLoading,
      'stageInfo': stageInfo,
  ...
  return (
    <>
      ...
      {/* 编辑弹窗 */}
      <SchemeStageEditModal
          projectId={projectId}
        visible={editModalVisible}
        stageInfo={stageInfo}
        onCancel={() => setEditModalVisible(false)}
          onSuccess={async () => {
          console.log('[SchemeStageFileManagement] onSuccess 回调被调用')
          setEditModalVisible(false)
          // 刷新查询缓存，确保数据更新
          console.log('[SchemeStageFileManagement] 开始刷新查询缓存')
          await queryClient.invalidateQueries({
            queryKey: ['productionFilesByStage', projectId, 'scheme'],
          })
          console.log('[SchemeStageFileManagement] 查询缓存已刷新，开始重新获取数据')
          const refreshedData = await refetch()
          console.log('[SchemeStageFileManagement] 重新获取到的数据:', refreshedData.data)
          }}
        />
    </>
  )
