...
interface CompletionFileUploadProps {
...
export const CompletionFileUpload = ({
  projectId,
  visible,
  onCancel,
  onSuccess,
}: CompletionFileUploadProps) => {
  ...
  const handleFormFinish = async (values: any) => {
    try {
      ...
      const uploadPromises = fileList.map(async (fileItem) => {
        ...
        formData.append('file_type', 'completion_report')
        formData.append('stage', 'completion')
      ...
      <Form form={form} layout="vertical" onFinish={handleFormFinish}>
        <Form.Item label="文件" required>
          ...
          <div style={{ marginTop: 8, color: '#999', fontSize: 12 }}>
            支持 PDF、DOC、DOCX、XLS、XLSX、ZIP、RAR 等格式，最大 100MB，可上传多个文件
          </div>
        </Form.Item>
