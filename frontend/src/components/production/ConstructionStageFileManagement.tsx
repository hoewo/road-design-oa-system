...
import { ConstructionStageEditModal } from './ConstructionStageEditModal'
import type { StageFileInfo, ProductionFile } from '@/types'
import dayjs from 'dayjs'

const { Text } = Typography

interface ConstructionStageFileManagementProps {
  projectId: string | number
}

export const ConstructionStageFileManagement = ({
  projectId,
}: ConstructionStageFileManagementProps) => {
  ...
  const {
    ...
    queryFn: async () => {
      console.log('[ConstructionStageFileManagement] 开始获取阶段文件信息, projectId:', projectId)
      const data = await productionService.getProductionFilesByStage(projectId, 'construction')
      console.log('[ConstructionStageFileManagement] 获取到的数据:', data)
      return data
    },
    enabled: !!projectId,
  })
  ...
  // 日志：数据状态变化
  useEffect(() => {
    console.log('[ConstructionStageFileManagement] 数据状态:', {
      projectId,
      isLoading,
      'stageInfo': stageInfo,
  ...
  return (
    <>
        ...
        {isEmpty ? (
          ...
          <>
            ...
            <div style={{ margin: '16px 0', padding: '16px', border: '1px solid #f0f0f0', background: '#fafafa', borderRadius: '8px' }}>
              ...
              {renderFileList(mainFiles, '施工图设计文件')}
            </div>
      ...
      {/* 编辑弹窗 */}
      <ConstructionStageEditModal
          projectId={projectId}
        visible={editModalVisible}
        stageInfo={stageInfo}
        onCancel={() => setEditModalVisible(false)}
          onSuccess={async () => {
          console.log('[ConstructionStageFileManagement] onSuccess 回调被调用')
          setEditModalVisible(false)
          // 刷新查询缓存，确保数据更新
          console.log('[ConstructionStageFileManagement] 开始刷新查询缓存')
          await queryClient.invalidateQueries({
            queryKey: ['productionFilesByStage', projectId, 'construction'],
          })
          console.log('[ConstructionStageFileManagement] 查询缓存已刷新，开始重新获取数据')
          const refreshedData = await refetch()
          console.log('[ConstructionStageFileManagement] 重新获取到的数据:', refreshedData.data)
          }}
        />
    </>
  )
